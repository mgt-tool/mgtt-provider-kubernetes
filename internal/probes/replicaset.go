...
func registerReplicaset(r *provider.Registry, c *shell.Client) {
	get := func(ctx context.Context, req provider.Request) (map[string]any, error) {
		return KubectlJSON(ctx, c, "-n", req.Namespace, "get", "replicaset", req.Name)
	}
	...
	r.Register("replicaset", map[string]provider.ProbeFn{
