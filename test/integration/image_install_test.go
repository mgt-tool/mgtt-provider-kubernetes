...
func TestImageInstall_Capabilities(t *testing.T) {
	...
	digest := fetchManifestDigest(t, registryPort, "mgtt-provider-kubernetes", "it")
	if digest == "" {
		t.Fatal("could not resolve manifest digest after push")
	}
	digestRef := fmt.Sprintf("localhost:%s/mgtt-provider-kubernetes@%s", registryPort, digest)
	...
	metaPath := filepath.Join(home, "providers", capsTestProviderName, ".mgtt-install.json")
	...
	// 9. Uninstall cleanly.
	uninst := exec.CommandContext(ctx, "mgtt", "provider", "uninstall", capsTestProviderName)
